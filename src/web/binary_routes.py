...
def download_all_binaries(network: str, docker_image: str, docker_version: str):
    ...
    network_dir = os.path.join(EXTRACTED_DIR, network)
    if not os.path.exists(network_dir):
        logger.debug(f"Network directory {network_dir} not found")
    ...
    if not version_dirs:
        logger.debug(f"No matching version directories found")
        ...
        try:
            ...
            if version_metadata and 'binary_paths' in version_metadata:
                binary_paths = [p.strip()
                                for p in version_metadata['binary_paths'].split(',')]
                logger.debug(f"Found binary paths in metadata: {binary_paths}")
        except Exception as e:
            logger.error(f"Error reading version metadata: {e}")
    ...
    if not binary_paths:
        ...
        logger.debug(f"Using binary files found in directory: {binary_files}")
