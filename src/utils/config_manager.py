...
import logging
from .helpers import safe_load_yaml, safe_write_yaml
...
class ConfigManager:
    ...
    def load_config(self, force_reload: bool = False) -> Dict:
        """Load configuration from local file or remote repository with caching.
        ...
        if self.config_repo:
            try:
                ...
                if raw_url:
                    logger.info(f"Fetching configuration from: {raw_url}")
                    headers = {}
